...
get_params_at_year_sc <- function(fit, year_sc_val) {
  ...
  loc0 <- get_first(c("location", "mu0"), NA_real_)
  loc1 <- get_first(c("location:year_sc", "mu1"), 0)
  sc0  <- get_first(c("scale", "sigma0"), NA_real_)
  sc1  <- get_first(c("scale:year_sc", "sigma1"), 0)
  sh0  <- get_first(c("shape", "xi0"), NA_real_)
  sh1  <- get_first(c("shape:year_sc", "xi1"), 0)
...
delta_rl_ci <- function(fit, rp, year_sc_val, alpha = 0.05) {
  ...
  for (i in seq_len(p)) {
    tu <- theta; td <- theta
    tu[i] <- theta[i] + eps; td[i] <- theta[i] - eps
    grad[i] <- (rl_fun(tu) - rl_fun(td)) / (2 * eps)
  }
...
plot_density_common_scale <- function(fit, bm, model_key, bandwidth = NULL) {
  dens_emp <- if (is.null(bandwidth)) {
    density(bm$Wmax, na.rm = TRUE)
  } else {
    density(bm$Wmax, na.rm = TRUE, bw = bandwidth)
  }
  ...
  plot(dens_emp, main = "", xlab = "Annual Max Wind Speed (m/s)", ylab = "Density", lwd = 1)
  lines(x_grid, dens_mod, lty = 2, lwd = 1.6, col = "blue")
  legend("topright", legend = c("Empirical", "Modeled"),
         lty = c(1, 2), lwd = c(1, 1.6), col = c("black", "blue"), bty = "n")
}
...
save_rl_selected_timevarying <- function(fit, bm, RPs, file, model_name) {
  ...
  ylim_rng <- range(c(bm$Wmax, rl_mat), na.rm = TRUE)
  plot(bm$year, bm$Wmax, type = "l", col = "grey70",
       xlab = "Year", ylab = "Annual Max Wind Speed (m/s)",
  ...
  legend("topright", legend = paste0(RPs, "-year"),
         col = cols, lty = 1, lwd = 2, bty = "n", inset = c(-0.25, 0), xpd = TRUE)
}
...
closest_year <- function(y, years_avail) years_avail[which.min(abs(years_avail - y))]


# 1) Data


dt <- as.data.table(Wind_the_kooy)
dt[, date := ymd(sprintf("%08d", as.integer(YYYYMMDD)))]
dt[, year := year(date)]

build_bm <- function(dt_in, wind_var = "FXX") {
  if (!wind_var %in% names(dt_in)) stop("Column not found: ", wind_var)
  dt_tmp <- copy(dt_in)
  dt_tmp[, wind_ms := suppressWarnings(as.numeric(get(wind_var))) / 10]
  y0 <- dt_tmp[is.finite(wind_ms), min(year)]
  if (!is.finite(y0)) {
    return(list(bm = data.table(year = integer(), Wmax = numeric()),
                start_year = NA_integer_, used_var = wind_var))
  }
  dta <- dt_tmp[year >= y0 & is.finite(wind_ms)]
  bm  <- dta[, .(Wmax = max(wind_ms, na.rm = TRUE)), by = year][order(year)]
  bm  <- bm[is.finite(Wmax)]
  list(bm = bm, start_year = as.integer(y0), used_var = wind_var)
}

res <- NULL
if ("FXX" %in% names(dt)) {
  res <- build_bm(dt, "FXX")
  if (nrow(res$bm) < 10 || uniqueN(res$bm$Wmax) < 3) res <- NULL
}
if (is.null(res) && "FHX" %in% names(dt)) {
  message("Using FHX (FXX missing/too sparse).")
  res <- build_bm(dt, "FHX")
}
...
used_var <- res$used_var

stopifnot(nrow(bm) >= 10, uniqueN(bm$Wmax) >= 3)


# 2) Block maxima + scaled time


yrs_ref <- min(bm$year):max(bm$year)
mu_y <- mean(yrs_ref)
sd_y <- sd(yrs_ref)
...
fit_list <- lapply(names(model_specs), function(nm) {
  sp <- model_specs[[nm]]
  fevd(Wmax, data = bm, type = "GEV", method = "MLE",
       location = sp$location, scale = sp$scale, shape = sp$shape)
})
names(fit_list) <- names(model_specs)
fit_stat   <- fit_list[["stat"]]
fit_mu     <- fit_list[["mu"]]
fit_sig    <- fit_list[["sig"]]
...
aic_tab <- do.call(rbind, lapply(names(fit_list), function(nm) {
  f <- fit_list[[nm]]
  data.frame(Model = nm, k = length(f$results$par), NLL = f$results$value)
}))
...
grab_ci_param <- function(fit) {
  ...
  if (!inherits(out, "try-error")) {
    ...
    data.frame(Parameter = names(est), Estimate = as.numeric(est),
               lower = as.numeric(M[, low_i]), upper = as.numeric(M[, up_i]))
  } else {
    covm <- fit$results$cov
    if (!is.null(covm) && length(diag(covm)) == length(est)) {
      se <- sqrt(diag(covm)); z <- qnorm(0.975)
      data.frame(Parameter = names(est), Estimate = as.numeric(est),
                 lower = as.numeric(est - z * se), upper = as.numeric(est + z * se))
    } else {
      data.frame(Parameter = names(est), Estimate = as.numeric(est), lower = NA_real_, upper = NA_real_)
    }
  }
}
...
# 6) Return levels at latest year with delta method CI for z_50


RPs <- c(2, 5, 10, 20, 50)
latest_year    <- max(bm$year)
year_latest_sc <- (latest_year - mu_y) / sd_y

rl_wide_tab <- do.call(rbind, lapply(names(fit_list), function(nm) {
  f <- fit_list[[nm]]
  vals <- sapply(RPs, function(rp) delta_rl_ci(f, rp, year_latest_sc)["estimate"])
  z50_ci <- delta_rl_ci(f, rp = 50, year_sc_val = year_latest_sc)
  data.frame(Model = nm, z2 = vals[1], z5 = vals[2], z10 = vals[3], z20 = vals[4],
             z50 = z50_ci["estimate"], z50_lower = z50_ci["lower"],
             z50_upper = z50_ci["upper"], z50_se = z50_ci["se"], row.names = NULL)
}))
rl_wide_tab <- rl_wide_tab[order(rl_wide_tab$Model), ]

years_check <- c(1960, 1990, latest_year)
years_check <- years_check[years_check >= min(bm$year) & years_check <= max(bm$year)]
years_check <- sort(unique(sapply(years_check, closest_year, years_avail = bm$year)))
year_index  <- match(years_check, bm$year)
year_index  <- year_index[!is.na(year_index)]

get_rl_years <- function(fit, rp, year_index, n_obs) {
  rl_all <- as.numeric(return.level(fit, return.period = rp, CI = FALSE))
  if (length(rl_all) == 1L) return(rep(rl_all, length(year_index)))
  if (length(rl_all) != n_obs) {
    warning(sprintf("RL length mismatch; got %d, expected %d.", length(rl_all), n_obs))
    return(rl_all[seq_along(year_index)])
  }
  rl_all[year_index]
}

rl_all_models_long <- data.table::rbindlist(lapply(names(fit_list), function(nm) {
  fit <- fit_list[[nm]]
  data.table::rbindlist(lapply(RPs, function(rp) {
    data.table::data.table(Model = nm, Year = bm$year[year_index], RP = rp,
                           RL = get_rl_years(fit, rp, year_index, nrow(bm)))
  }))
}))
rl_all_models_long <- rl_all_models_long[order(Model, Year, RP)]


# 7) Save outputs


base_out <- sprintf("bm_output_dekooy_wind_%s_FIXED", tolower(used_var))
dir.create(base_out, showWarnings = FALSE)

fwrite(bm[, .(year, Wmax)], file.path(base_out, "DeKooy_BM_Wind.csv"))
...
fwrite(rl_wide_tab,        file.path(base_out, sprintf("ReturnLevels_WIDE_all_models_%d.csv", latest_year)))
fwrite(rl_all_models_long, file.path(base_out, "ReturnLevels_ALLMODELS_years_long.csv"))
...
plot(bm$year, bm$Wmax, pch = 16, xlab = "Year", ylab = "Annual Max Wind Speed (m/s)",
     main = sprintf("De Kooy Annual Maximum Wind Speed (%d\u2013%d) [%s]",
                    min(bm$year), max(bm$year), used_var))
abline(lm(Wmax ~ year, bm), lty = 2)
dev.off()
...
cat("\n=== De Kooy Wind: AIC/BIC (sorted by AIC) ===\n")
print(aic_tab, row.names = FALSE)
cat("\n=== LRT key comparisons ===\n")
print(lrt_tab, row.names = FALSE)
cat("\n=== Return levels at latest year with delta method 95% CI for z_50 ===\n")
