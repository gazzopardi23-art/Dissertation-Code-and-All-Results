...
plot_density_common_scale <- function(fit, bm, model_key, bandwidth = NULL) {
  dens_emp <- if (is.null(bandwidth)) {
    density(bm$Pmax, na.rm = TRUE)
  } else {
    density(bm$Pmax, na.rm = TRUE, bw = bandwidth)
  }
  ...
  legend("topright", legend = c("Empirical", "Modeled"), lty = c(1, 2), lwd = c(1, 1.6),
...
save_rl_selected_timevarying <- function(fit, bm, RPs, file, model_name) {
  ...
  plot(bm$year, bm$Pmax, type = "l", col = "grey70",
       xlab = "Year", ylab = "Annual Max Daily Precipitation (mm)",
...
dt <- as.data.table(precipitation_data_de_bilt_1950_2024_)
dt[, date      := ymd(sprintf("%08d", as.integer(`YYYYMMDD`)))]
dt[, precip_mm := `DAILY SUM OF PRECIPITATION` / 10]
dt[, year      := year(date)]
...
bm <- dt[year %between% c(1950, 2024),
         .(Pmax = max(precip_mm, na.rm = TRUE)),
         by = year][order(year)]
...
fit_list <- lapply(names(model_specs), function(nm) {
  sp <- model_specs[[nm]]
  fevd(Pmax, data = bm, type = "GEV", method = "MLE",
...
base_out <- "bm_output_debilt_precip_FIXED"; dir.create(base_out, showWarnings = FALSE)
fwrite(bm[, .(year, Pmax)], file.path(base_out, "DeBilt_BM_Precip_1950_2024.csv"))
...
plot(bm$year, bm$Pmax, pch = 16, xlab = "Year", ylab = "Annual Max Daily Precipitation (mm)",
     main = "De Bilt Annual Maximum Daily Precipitation (1950\u20132024)")
