...
plot_density_common_scale <- function(fit, bm, model_key, bandwidth = NULL) {
  dens_emp <- if (is.null(bandwidth)) {
    density(bm$Ycold, na.rm = TRUE)
  } else {
    density(bm$Ycold, na.rm = TRUE, bw = bandwidth)
  }
  ...
  if (model_key == "stat") {
    ...
    dens_matrix <- sapply(bm$year_sc, function(ysc) {
      params <- tryCatch(get_params_at_year_sc(fit, year_sc_val = ysc), error = function(e) NULL)
...
save_rl_selected_timevarying_Tmin <- function(fit, bm_min, RPs, file, model_name) {
  ...
  plot(bm_min$year, bm_min$Tmin, type = "l", col = "grey70",
       xlab = "Year", ylab = "Annual Tmin (\u00b0C)",
  ...
  legend("topright", legend = paste0(RPs, "-year"), col = cols, lty = 1, lwd = 2, bty = "n",
...
bm_min <- dt[year %between% c(1950, 2024), .(Tmin = min(tn_c, na.rm = TRUE)), by = year]
bm_min[, Ycold := -Tmin]
yrs_ref <- 1950:2024; mu_y <- mean(yrs_ref); sd_y <- sd(yrs_ref)
...
fit_list_min <- lapply(names(model_specs), function(nm) {
  sp <- model_specs[[nm]]
  fevd(Ycold, data = bm_min, type = "GEV", method = "MLE",
       location = sp$location, scale = sp$scale, shape = sp$shape)
})
names(fit_list_min) <- names(model_specs)
fit_stat_min   <- fit_list_min[["stat"]]
fit_mu_min     <- fit_list_min[["mu"]]
fit_mu_sig_min <- fit_list_min[["mu_sig"]]
...
base_out <- "bm_output_debilt_minima_FIXED"; dir.create(base_out, showWarnings = FALSE)
fwrite(bm_min[, .(year, Tmin, Ycold)], file.path(base_out, "DeBilt_BMin_Tmin_1950_2024.csv"))
...
plot(bm_min$year, bm_min$Tmin, pch = 16, xlab = "Year", ylab = "Annual Tmin (\u00b0C)",
     main = "De Bilt Annual Minimum Temperature (1950\u20132024)")
...
cat("\n=== MINIMA (via Y = -Tmin) AIC/BIC (sorted by AIC) ===\n")
