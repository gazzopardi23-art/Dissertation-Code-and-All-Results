...
save_rl_selected_timevarying <- function(fit, bm, RPs, file, model_name) {
  ...
  plot(bm$year, bm$Wmax, type = "l", col = "grey70",
       xlab = "Year", ylab = "Annual Max Wind Gust (m/s)",
...
dt <- as.data.table(Daily_Wind_Speed_Data_De_Bilt_Station_1950_2024_)
...
bm <- dt[year %between% c(1950, 2024) & is.finite(FXX_ms),
         .(Wmax = max(FXX_ms, na.rm = TRUE)),
         by = year][order(year)]
...
base_out <- "bm_output_debilt_wind_FIXED"
dir.create(base_out, showWarnings = FALSE)

fwrite(bm[, .(year, Wmax)], file.path(base_out, "DeBilt_BM_Wind_1950_2024.csv"))
...
plot(bm$year, bm$Wmax, pch = 16,
     xlab = "Year", ylab = "Annual Max Wind Gust (m/s)",
     main = "De Bilt Annual Maximum Wind Gust (1950\u20132024)")
