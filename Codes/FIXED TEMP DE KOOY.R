...
plot_density_common_scale <- function(fit, bm, model_key, bandwidth = NULL) {
  ...
  plot(dens_emp, main = "", xlab = "Annual Max Temperature (\u00b0C)", ylab = "Density", lwd = 1)
...
bm <- dt[is.finite(tx_c),
         .(Tmax = max(tx_c, na.rm = TRUE)),
         by = year][order(year)]
bm <- bm[is.finite(Tmax)]
...
base_out <- "bm_output_dekooy_tmax_FIXED"; dir.create(base_out, showWarnings = FALSE)
fwrite(bm[, .(year, Tmax)], file.path(base_out, "DeKooy_BM_Tmax.csv"))
...
plot(bm$year, bm$Tmax, pch = 16, xlab = "Year", ylab = "Annual Max Temperature (\u00b0C)",
     main = sprintf("De Kooy Annual Maximum Temperature (%d\u2013%d)", min(bm$year), max(bm$year)))
abline(lm(Tmax ~ year, bm), lty = 2); dev.off()
...
cat("\n=== De Kooy TX: AIC/BIC (sorted by AIC) ===\n")
